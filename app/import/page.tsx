...
const FIELD_DEFS: Record<TableKey, DbField[]> = {
  ...
  calendar: [
    { key: "title", label: "Název události", required: true },
    { key: "date", label: "Datum", required: true, hint: "YYYY-MM-DD nebo DD.MM.YYYY" },
    { key: "time_from", label: "Čas od", hint: "HH:MM (volitelné, výchozí 09:00)" },
    { key: "time_to", label: "Čas do", hint: "HH:MM (volitelné, výchozí +1h)" },
    { key: "start_time", label: "Začátek (plný)", hint: "YYYY-MM-DD HH:MM — alternativa k datum+čas" },
    { key: "end_time", label: "Konec (plný)", hint: "YYYY-MM-DD HH:MM — alternativa k datum+čas" },
    { key: "type", label: "Typ", hint: "prohlídka / meeting / hovor / jiné" },
    { key: "location", label: "Místo" },
    { key: "notes", label: "Poznámky" },
    { key: "client_name", label: "Klient (jméno)", hint: "vyhledá klienta podle jména" },
    { key: "property_address", label: "Nemovitost (adresa)", hint: "vyhledá nemovitost podle adresy" },
  ],
};
...
function autoGuessMapping(columns: string[], fields: DbField[]): Record<string, string> {
  ...
  const SYNONYMS: Record<string, string[]> = {
    ...
    title: ["nazev", "titulek", "title", "udalost", "nazev_udalosti", "nazev_akce", "akce"],
    date: ["datum", "date", "den"],
    time_from: ["cas_od", "time_from", "od", "zacatek_cas", "start"],
    time_to: ["cas_do", "time_to", "do", "konec_cas", "end"],
    start_time: ["start_time", "zacatek", "datum_zacatek", "datum_a_cas_od"],
    end_time: ["end_time", "konec", "datum_konec", "datum_a_cas_do"],
    location: ["misto", "adresa_mista", "location", "misto_konani"],
